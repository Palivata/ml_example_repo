...
def stratify_shuffle_split_subsets(
    ...
    train_indexes, else_indexes = _split(
        all_x, all_y, distribution=[1 - train_fraction, train_fraction]
    )
    ...
    train_subset = pd.DataFrame(
        data=np.concatenate([x_train, y_train], axis=1), columns=x_columns + y_columns
    )
    valid_subset = pd.DataFrame(
        data=np.concatenate([x_valid, y_valid], axis=1), columns=x_columns + y_columns
    )
    test_subset = pd.DataFrame(
        data=np.concatenate([x_test, y_test], axis=1), columns=x_columns + y_columns
    )
